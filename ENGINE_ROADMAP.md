...
- [x] 씬의 충돌 지오메트리로부터 빌드 — ⚠️ 2026-09-23 정정: 이 항목이 닫힐 때 있던 것은
  발자국→폴리곤 프리미티브(`build_from_footprints`)뿐이었고, 호출처는 자기 테스트뿐이었다.
  콜라이더 → 발자국 → `NavMesh` 경로(`NavMeshSurface` 컴포넌트, `Bsengine.navmesh.bake`)는
  2026-09-23 후속 PR에서 완성됐다. 이 체크박스를 근거로 "이미 있다"고 판단했던 것이 비교
  문서가 경고하는 함정의 한 사례다.
...
**미뤘다가 기반부터 깔았다.** 처음 착수하려 했을 때 "작업의 절반이 관찰할 수 없는 GPU
파이프라인"이라 보고했고, 그 판단이 item 36(픽셀 리드백)·37(투명도)·38(텍스처 배관)을
낳았다. 그 셋이 없었다면 파티클을 짜면서 **방향광이 죽어 있는 것도, 스카이박스가 크래시하는
것도, 색을 씬에서 저작할 수 없는 것도** 모르고 지나갔을 것이다.

**완료 조건:**
- [x] `ParticleEmitter` 컴포넌트 — 파라미터는 리플렉트, 살아있는 파티클·버스트 카운터·
      난수·분수 캐리는 `#[reflect(ignore)]` (`SkinnedMesh`와 같은 방식)
- [x] CPU 시뮬레이션(`bsengine-app`) + 정점 버퍼 없는 인스턴스 빌보드 렌더링.
      패스는 item 37의 투과 패스 뒤, 알파 블렌딩·깊이 테스트 켬·쓰기 끔
- [x] 씬 RON과 스크립팅(`Bsengine.burstParticles`) 양쪽으로 부착
- [x] `games/mini-arena` 피격 스파크 — 공격이 맞은 지점에서 튄다
- [x] 테스트 20개(컴포넌트 5, 시뮬레이션 7, op 2, 픽셀 4, 콘텐츠 2), 변이 검증

**`particle_lifetime`이라 부르는 이유.** 카탈로그가 `lifetime`의 소유자로 기존 `Lifetime`
컴포넌트를 답했고 그건 "엔티티를 despawn한다"는 뜻이다. 이미터 필드가 `lifetime`이면
"이 이미터가 사라진다"로 읽힌다.

**난수가 결정적이다.** 리플레이는 재현성을 위해 시계를 고정하므로 파티클 방향도 반복
가능해야 한다. 이미터마다 시드를 갖는 xorshift를 쓰고 `rand` 크레이트를 들이지 않았다.

---

#### 이 항목이 찾아낸 것 둘

**1. `LifetimePlugin`이 어느 호스트에도 설치돼 있지 않았다.** 정의되고 재수출까지 됐는데
아무도 달지 않아서, `Bsengine.setLifetime()`은 **op와 JS 바인딩과 op 단위 테스트까지 갖춘
스크립팅 API인데 실행 중인 게임에서 한 번도 엔티티를 despawn한 적이 없다.**
`ParticlePlugin`의 선례로 베끼러 갔다가 그 선례가 죽어 있는 것을 발견했다. 둘 다 창 있는
런타임과 헤드리스 앱 양쪽에 달았다 — item 11/12가 기록한 그 비대칭이다.

가드는 **"플러그인이 목록에 있는가"가 아니라 "동작이 일어나는가"**다. 전자는 원래 버그를
잡을 수 없다 — 확인할 목록 항목 자체가 없었다.

**2. `ReflectColor`에 `ReflectDeserialize`가 없었다.** 그래서 **색을 가진 리플렉트 컴포넌트는
씬에서 저작할 수 없었다** — 경고 하나 남기고 컴포넌트 전체가 조용히 사라진다. `Material`의
두 색을 포함해 엔진의 모든 색 필드가 해당된다. 세 f32로 직렬화하도록 손으로 구현했고
(glam의 serde 기능은 계속 꺼 둔다), 형제 래퍼들(`ReflectVec3` 등)의 같은 갭은 **씬이
저작하지 않으므로** 고치지 않고 기록만 남긴다.

**변이가 내 믿음을 하나 정정했다.** RON에서 필드가 빠지는 것은 허용된다 — 값이 `Default`
위에 부분 적용되기 때문이다. item 29의 조용한 비움은 *컬렉션 원소* 타입이 필드를 얻은
경우였고, 그것과 최상위 필드 누락은 다른 실패다.

**알려진 한계:** 파티클끼리는 정렬하지 않는다(깊이 쓰기가 꺼져 있어 서로 가리지 않는다),
이미터당 4096·프레임당 16384 상한(초과분은 경고 후 버림), 텍스처는 이미터 단위,
빌보드는 카메라를 향할 뿐 속도 방향으로 늘어나지 않는다.
...
**완료 조건:**
- [x] `Output` enum으로 창/오프스크린을 가르고 `new_offscreen(w, h)` 추가 — 파이프라인
      구축은 창 경로와 **공유한다**. 복제본을 만들면 테스트가 진짜 렌더러를 검사하지 않는다
- [x] `output.rs`의 `read_pixels()` — 행 256바이트 정렬. 테스트 해상도는 200×150
      (`200*4 = 800`, `800 % 256 = 32`이라 패딩 경로를 강제한다. 폭이 64의 배수면
      패딩을 빠뜨린 코드도 통과한다)
- [x] 픽셀 테스트 **15개** — 기본 4, 라이팅·그림자 4, 재질 3, 포스트프로세스 3, UI 1.
      각각 변이로 검증
- [x] 어댑터가 그리지 못하면 어댑터 이름과 함께 실패 — **조용한 skip 금지**
- [x] 창 경로 회귀 없음(E2E 8개, 에디터 825개), CI 양쪽 러너에서 픽셀 테스트 통과

**CI는 그린다.** Ubuntu의 lavapipe와 Windows 어댑터 모두 래스터라이즈했고, 클리어 색
측정값이 세 환경(내 GPU, lavapipe, Windows)에서 허용 오차 6 안에 들어왔다. 소프트웨어
래스터라이저와 실제 GPU가 같은 답을 낸다는 뜻이라, 부등식 위주로만 쓸 필요는 없었다.

---

#### 이 항목이 찾아낸 것 — 픽셀을 본 첫날에

**1. 방향광이 한 번도 동작한 적이 없다.** 그림자 비교 샘플러가 `GreaterEqual`인데
섀도우 패스는 표준 forward-Z(1.0 클리어, `depth_compare: Less`, 기울기 바이어스)로
쓴다. 부등호가 뒤집혀 `shadow_factor`가 섀도우 절두체 안의 모든 프래그먼트에 0을
돌려줬고, 방향광 항 전체가 거기에 곱해진다. `compute_light_view_proj`가 원점 중심
60유닛 상자를 쓰므로 **다섯 게임의 모든 물체**가 그 안이다. 즉 모든 장면이 ambient
만으로 렌더링돼 왔고 그림자는 한 번도 그려진 적이 없다.

측정으로 확인했다: ambient를 끄면 어떤 광원 각도에서도 화면이 검정, 태양광을 4배로
올려도 한 바이트도 안 바뀜, 섀도우 맵을 빈 곳으로 겨누면(범위 밖 조기 반환 1.0)
140으로 돌아옴. 고친 뒤 열린 바닥 0 → 128, 떠 있는 큐브가 바닥을 31로 어둡게 만든다.

**버그가 스스로를 가렸다.** 태양광 기여가 0이면 "전부 그림자"와 "그림자 없음"이 같은
그림이다. 그리고 E2E 8개는 픽셀을 보지 않으므로 조명이 통째로 죽어 있어도 전부 초록이었다.

**2. 스카이박스 파이프라인이 잘못된 버퍼를 겨눴다.** 스카이박스 패스는 HDR 버퍼
(`Rgba16Float`)에 그리는데 파이프라인은 최종 출력 포맷으로 만들어졌다. 스카이박스를
저작한 프로젝트는 첫 프레임에서 검증 에러로 죽는다. 여기 게임 중 쓰는 것이 없어 아무도
밟지 않았다. 바로 옆 `compile_and_store_shader`는 같은 이유로 이미 `HDR_FORMAT`을
올바르게 쓰고 있었다.

**테스트 자체가 두 번 틀렸다.** 그림자 테스트 둘은 처음에 "큐브를 넣으면 프레임이 얼마나
어두워지는가"를 쟀는데, 그건 큐브가 바닥을 **가리는** 것만으로도 만족된다 — 버그가 있는
채로 통과했다. 지금은 큐브 실루엣을 실제로 계산해 제외하고 한 프레임 안에서 비교한다.
블룸 테스트는 물체 마스크를 밝기 임계값으로 잡았다가 배경 클리어 색(luma 85)이 전부
잡히는 바람에 검사할 픽셀이 하나도 안 남았다.
...
**완료 조건:**
- [x] `Material.opacity: f32` — `metallic`/`roughness`와 같은 결이다. `base_color`를
      Vec4로 바꾸지 않는다: `ReflectColor`는 `emissive`와 라이트 색이 공유하는데 거기
      알파는 의미가 없다
- [x] `EntityDescriptor`에 `opacity` — `#[serde(default)] Option<f32>`로 #1769의 감쇠
      필드와 같은 모양. 기존 씬이 그대로 파싱되고 **0이 아니라 "의견 없음"으로** 온다
- [x] 알파 블렌드 파이프라인 + 뒤에서 앞으로 정렬된 투과 패스 (깊이 테스트 켬, 깊이 쓰기 끔).
      순서는 불투명 → 스카이박스 → 투과 → 포스트프로세스
- [x] mini-arena의 Pickup이 유리구슬
- [x] 36의 리드백으로 검증 — 픽셀 테스트 4개, 변이 3개가 각각 정확히 하나씩 무너뜨린다
- [x] 알려진 한계 기록 (아래)

**계획이 틀렸던 것 하나.** "`Material` 리터럴 8곳을 컴파일러가 잡아 준다"고 적었는데
실측은 10곳이었고, **그중 고칠 것은 0곳이었다** — 전부 `..Default::default()`를 쓰고 있어
기본값 있는 새 필드가 아예 보이지 않는다. 예상보다 안전했지만 근거는 틀렸다.

**계획에 없었는데 해야 했던 것.** 에디터의 씬 **저장** 경로는 `Material`을 리플렉트로
읽지 않고 `InspectorState`를 거치는 손으로 이어진 사슬(4곳)이다. 인스펙터 *표시*는
`Material`이 리플렉트라 공짜로 되지만, 저장 경로를 안 이으면 **에디터로 씬을 열어 저장하는
순간 투명도가 조용히 사라진다.** 이번 세션이 반복해서 찾아낸 종류라 지금 이었다.

**E2E가 확인할 수 없는 것.** 리플레이 하니스의 질의는 위치·가시성·HUD 텍스트·에셋 상태뿐이라
재질을 볼 수 없다. "Pickup이 유리다"는 어떤 녹화도 확인할 수 없는 주장이라 별도의 Rust
가드 테스트를 뒀다(`bsengine-runtime/tests/game_content.rs`).

**알려진 한계 — 전부 결정이지 누락이 아니다:**
- **투명 물체도 불투명한 그림자를 드리운다.** 섀도우 패스는 모든 드로우를 그린다. 반투명
  그림자는 별개 주제이고, 투명 드로우를 섀도우 패스에서 빼면 유리 밑이 완전히 밝아져
  그것대로 틀려 보인다.
- **커스텀 셰이더를 쓰는 드로우는 블렌딩되지 않는다.** 투과 패스에서도 자기 파이프라인을
  쓴다. 커스텀 셰이더는 알파를 포함해 자기 출력을 스스로 정한다.
- **정렬은 물체 원점 기준이다.** 서로 관통하거나 아주 큰 투명 메시는 순서가 틀릴 수 있다.
...
**완료 조건:**
- [x] `EntityDescriptor.texture: Option<AssetRef>` — `gltf`/`script`와 같은 guid+경로
      형식. raw 경로면 item 30의 리네임 내성에 구멍이 난다. 기존 해석 패스에 합류
- [x] 해석된 경로를 나르는 `TexturePath(String)` 컴포넌트. **`bsengine-core`의 `Material`
      옆에 둔다** — 씬 크레이트에 두면 `bsengine-render`가 씬 *파일 포맷*에 의존하게
      되는데, 렌더링이 알 필요 없는 것이다. core에 두면 새 간선이 아예 없다
      (씬·렌더·에디터 셋 다 이미 core에 의존)
- [x] `bsengine-render`의 `TextureCache` — 경로별 상태, `Ready`가 핸들 보유(놓으면
      핫리로드가 조용히 꺼진다). **같은 경로는 한 번만 업로드**
- [x] 실패는 `LoadState::Failed` → `GaveUp` + 한 번 경고
- [x] 체커보드 텍스처 생성, mini-arena의 바닥에 적용
- [x] 테스트 각 층 변이 검증, E2E 8개·픽셀 19개 회귀 없음

**계획이 틀렸던 것 둘.**

**`Material` 리터럴은 컴파일러가 잡아 주지 않았다.** 전부 `..Default::default()`를
쓰고 있어 새 필드가 보이지도 않는다 — item 37에서 배운 것과 같다.

**E2E로 검증할 수 없다.** 계획은 녹화에 `get_asset_status` 단언을 넣으려 했는데,
헤드리스 테스트 앱은 **의도적으로 `RenderPlugin` 없이** 돌아서 텍스처를 아무것도
요청하지 않는다. `"unknown"`이 옳은 답이고, 그 부재는 `test_mode.rs`에 이미 주석으로
적혀 있었다 — **계획이 코드베이스가 이미 반박해 둔 가정 위에 서 있었다.** 대신 item 37의
유리구슬과 같은 Rust 콘텐츠 가드를 뒀다.

**변이가 테스트가 아니라 코드의 결함을 알려준 경우.** 업로드 직후 `texture_id`를 쓰는
줄을 지워도 모든 테스트가 통과했다. 테스트가 약해서가 아니라 **다음 프레임에 `Ready`
분기가 똑같이 처리하므로 관측 가능한 동작이 같기 때문**이었다. 줄을 지웠다.

**드러난 사실 하나.** 경로→에셋 상태 기계가 `bsengine-render`에 **이미 둘**(스카이박스,
커스텀 셰이더) 있고 이것이 셋째다. 새 모양을 발명하지 않고 기존 둘과 같은 형태를 썼다.
**셋을 하나로 합치기는 별도 항목으로 승격할 가치가 있다.**

---

### 39. 경로→에셋 상태 기계를 하나로 (`AssetSlot`) ✅

**목표:** "요청은 한 번, 폴링만, 실패는 종착"이 여러 곳에 각자 구현돼 있는 것을 한 타입으로.

설계: [docs/superpowers/specs/2026-08-07-asset-slot-design.md](docs/superpowers/specs/2026-08-07-asset-slot-design.md)

item 38에서 "셋이 있고 합칠 가치가 있다"고 적었는데, 세어 보니 **다섯**이었다:
`PendingGltf`, `ScriptLoadState`, `PendingSkyboxState`, `PendingShader`, `TextureState`.

**그리고 다섯 모두 이미 실패 경로 테스트를 갖고 있다** — 이름까지 나란하다
(`missing_*_is_given_up_on_instead_of_retried_forever`). 즉 **중복은 정확성 위험이
아니었다.** 이 항목이 없애는 것은 위험이 아니라 같은 설명이 다섯 번 재발명된 상태다.
근거가 약해졌다는 점을 사용자에게 보고했고, 균일성을 위해 다섯 전부 통합하기로 결정했다.

**완료 조건:**
- [x] `bsengine-asset`에 `AssetSlot<A>` — Loading / Ready / GaveUp, `poll`이 전이를 반환
- [x] 다섯 사이트 이전, 각 이전마다 **그 사이트의 기존 테스트가 초록**
- [x] 슬롯 자체 테스트 4개, 각각 변이 검증
- [x] E2E·픽셀 회귀 없음, CI 통과

**계획한 설계는 틀렸고, 코드를 읽고서야 알았다.** 첫 판은 `Ready { value: R }`에 GPU
작업(컴파일·업로드)을 넣는 4상태 슬롯이었다. 그런데 셰이더와 스카이박스는 **둘 다 GPU가
없어도 에셋이 도착하면 `Ready`로 가고**, 두 주석이 같은 말을 적어 두고 있었다 — "Says
nothing about the *compile* / *upload*, which needs a GPU the load does not". 그리고
`rebuild_modified_shaders`는 `Loading`을 벗어난 경로만 훑는다. **GPU 작업을 슬롯에 넣으면
서피스가 없는 동안 핫리로드가 꺼진다.** 승인받은 설계를 스스로 뒤집고 3상태로 고쳤다.
`CompileFailed`는 슬롯으로 올라가지 않는다 — 컴파일은 로드가 아니라 **로드 이후**의 사실이다.

**`poll`이 상태가 아니라 전이를 반환하는 이유.** 다섯 곳이 공통으로 하는 일은 "도착하면 한
번만 무언가 하고, 실패하면 한 번만 경고"다. 상태를 반환하면 호출자마다 "이미 처리했는가"를
따로 기억해야 한다. 전이를 반환하면 슬롯의 상태가 곧 그 기억이 되고, 스크립팅이 `Loading`을
그 용도로 쓰던 것이 그대로 사라진다.

**스크립팅이 설계 공백을 잡았다.** `reexecute_modified_scripts`는 `GaveUp` 스크립트도 파일이
나중에 생기면 되살리고, 그건 실패한 로드의 핸들을 계속 쥐고 있어야 가능하다 — 처음 만든
`GaveUp`은 핸들을 버렸다. `GaveUp(Handle)`로 고쳤고, `mark_arrived`(이벤트로 알게 된 도착을
기록, 재요청은 아님)를 더했다.

**변이 테스트가 찾은 것 넷.**

1. **`missing_skybox`·`missing_texture`·`missing_shader` 세 테스트가 실패할 수 없었다.**
   N 프레임 뒤 상태를 한 번 읽었는데, **매 프레임 재요청하는 루프도 그 순간엔 `GaveUp`에
   있다.** 이름이 내건 "retried forever"와 정확히 구분이 안 되는 단언이었다. 이후 60프레임
   동안 매 프레임 유지되는지 보도록 고쳤고, 그 판은 재요청을 넣으면 실패한다.
2. **`a_shader_that_failed_to_compile_..._but_stays_reloadable`의 이름 절반이 미측정이었다.**
   파일을 수정하지 않으므로 `rebuild_modified_shaders`가 아예 돌지 않는다 — 그 시스템이 깨진
   셰이더를 건너뛰게 만들어도 초록이었다. 진짜 서피스는 테스트에서 못 만드므로 선택 조건을
   `wants_rebuild`로 빼서 직접 쟀다.
3. **내가 넣은 스카이박스 `uploaded` 플래그가 죽은 코드였다.** `set_loaded_skybox_path`가
   dedupe를 성립시키는 것이 재업로드를 막는 실체였다. 게다가 서피스가 재생성되면 그 플래그가
   **정당한 재업로드를 막았을** 것이다 — 테스트는 통과하면서.
4. `mark_arrived`를 no-op으로 만들면 복구 테스트가 실패한다 — 그 메서드가 실제로 일한다.

**남은 것 하나: `SoundLoad`.** 오디오 재생 경로에 **여섯 번째** 같은 모양이 있다
(`bsengine-scripting`, `HashMap<String, SoundLoad>`). 다만 **요청 자체가 실패할 수 있어**
핸들 없는 `GaveUp`이 필요하고, `AssetSlot::GaveUp(Handle)`에 그대로 들어가지 않는다.
범위를 조용히 넓히지 않고 남겨 둔다.

**이득의 크기를 정직하게.** 명백한 이득은 스카이박스 하나(enum이 통째로 사라짐)다.
셰이더·텍스처·스크립팅은 대체로 본전이고, glTF는 상태가 없어도 됐던 곳이 상태를 갖는 **손해**다.
...
45개 전부 JS에 바인딩돼 있었지만 **바인딩 블록 밖에서는 아무도 호출하지 않았다** — 게임
스크립트·테스트·MCP 툴·문서 어디에도 없었다.

**완료 조건:**
- [x] 삭제로 접근성을 잃는 `getFollowOffset`/`getLookAtUp`의 통짜 형태를 **먼저** 별도 커밋으로
- [x] op 45 + JS 바인딩 45줄 + `ScriptCommand` 변형 24개와 match arm 24개를 한 커밋에서
- [x] `axis_ops_baseline.txt`를 비워 R2가 이제 "하나도 없음"을 지킨다
- [x] op 300 → 257, `ops.rs` 10,702 → 10,223줄

**빠뜨리면 조용히 잃었을 것 하나.** 축별 게터의 테스트를 지우다 보니
`test_follow_read_ops`/`test_look_at_component_read_ops`가 걸렸는데, 이 둘은 **삭제되지 않는**
`getFollowTarget`·`getFollowSpeed`·`getLookAtTarget`도 재고 있었다. 통째로 지웠다면 살아남는
op 셋의 커버리지가 사라졌을 것이다. 새 게터로 옮겨 복원했다.

**부수적으로 고쳐진 것.** 축별 게터는 `Follow`가 없는 엔티티에 `0.0`을 반환해 **진짜 0과
구분이 되지 않았다.** 통짜 게터는 다른 게터들처럼 `null`을 준다.

**카탈로그 테스트 하나가 옛 숫자를 박아 두고 있었다.** `velocity_has_exactly_one_owner...`가
`hits.ops.len() >= 10`을 단언했는데, 이는 축별 velocity op 12개가 있던 시절의 수다. 주석이 밝힌
의도는 "컴포넌트 소유자가 없고 op 가족으로 노출된다"이지 개수가 아니므로, 가족 구성원 6개를
**이름으로** 단언하게 바꿨다.
...
네 후보가 모두 닫혔다. A는 item 33(운동학 스택 제거), B는 필드 이름을 `position`으로 통일,
C는 합치지 않고 방향을 명시, 그리고 **축별 op 45개 축소는 item 41+42가 되었다** — 설계하러
들어갔더니 줄일 것이 op 수가 아니라 3-벡터 방언 셋이었고, 그 과정에서 한 번도 동작한 적 없는
데모 하나가 드러났다.

`axis_ops_baseline.txt`가 비었으므로 R2는 이제 개수를 동결하는 래칫이 아니라 축별 op를
금지하는 규칙이다.

---

### 43. 프레임 프로파일러 / GPU 디버거 ✅

**목표:** 프레임 타이밍과 GPU 패스별 비용을 실시간으로 관측해, 이후 항목(터레인/GI/LOD 등)이
실제로 성능에 미치는 영향을 측정 가능하게 한다.

**완료 조건:**
- [x] CPU 프레임 타임 + wgpu 패스별(섀도우/포인트섀도우/메인/스카이/투명도/egui) GPU
      타임스탬프 쿼리 수집 — `wgpu::Features::TIMESTAMP_QUERY` 지원 어댑터에서만, 미지원
      시 그레이스풀 디그레이드
- [x] 에디터에 프로파일러 패널 추가 — 프레임 그래프 + 패스별 막대 그래프
- [x] draw call/삼각형 수/텍스처 메모리 등 기본 통계 노출
- [x] 헤드리스 테스트 모드에서도 통계를 조회 가능(MCP `get_frame_stats`)
- [x] 테스트 신규 10개 추가(`profiler.rs` 유닛 3, `tests/profiler_stats.rs` 통합 5,
      `get_frame_stats` MCP 2), CI 통과

**포스트프로세스 3개 패스는 타이밍 대상에서 제외했다.** `PostProcessState::apply`는
`WgpuSurface`와 분리된 `impl` 블록이라 타임스탬프 쿼리 세트에 직접 접근할 방법이 없고,
그걸 관통시키는 건 이 항목의 스코프 밖으로 판단했다 — bloom/SSAO/composite는 draw
call/삼각형 카운트에는 여전히 포함되지만 GPU 패스별 타이밍 목록에는 나타나지 않는다.
재검토가 필요해지면(예: 포스트프로세스 자체가 병목으로 의심되면) 별도로 다룬다.

**포인트라이트 섀도우는 하나의 애그리게이트 타이밍이다.** 활성 포인트라이트 수만큼
`begin_render_pass`가 반복되는데(라이트당 큐브 면 6개), 매번 쿼리 슬롯을 쓰는 대신
루프 전체를 감싸는 타임스탬프 하나(`"point_shadow"`)로 묶었다 — 개별 페이스별 비용은
안 보이지만 쿼리 세트가 무한정 커지지 않는다.

---

### 44. 터레인 시스템

**목표:** 높이맵 기반 대규모 지형을 렌더링/충돌 처리하여, 지금까지 실내·아레나형으로
국한된 씬 규모를 벗어난다.

**3개 하위 스텝으로 분리해 순차 진행 — 전부 완료.** (각각 독립된 브레인스토밍→spec→plan→구현
사이클): (1) 터레인 코어(청크+콜라이더+데모 검증) — 완료, (2) 텍스처 스플래팅 — 완료,
(3) 에디터 브러시 툴 — 완료.

**완료 조건:**
- [x] `Terrain` 컴포넌트 — 높이맵 데이터 + 청크 단위 메시 생성 (sub-step 1/3)
- [x] 최소 하나의 텍스처 스플래팅(레이어 블렌딩) 지원 (sub-step 2/3 — 4레이어
      grass/rock/dirt/snow, 높이/경사 기반 smoothstep 블렌딩, `games/terrain-demo`에서
      실제 렌더링 픽셀로 검증)
- [x] Rapier 충돌체 자동 생성(하이트필드 콜라이더) (sub-step 1/3)
- [x] 에디터에서 높이맵 임포트 + 브러시로 높이/텍스처 편집 가능 (sub-step 3/3 — 높이맵
      임포트는 sub-step 1/3, 뷰포트 브러시 툴로 높이 조각+텍스처 페인팅 모두 지원.
      스크린→월드 레이 피킹 + 기존 Rapier 레이캐스트 재사용, 드래그 중 라이브
      프리뷰(GpuMeshRegistry::update_vertices / GpuTextureRegistry::replace), 드래그
      종료 시 콜라이더 재생성(PhysicsWorld::set_collider_shape, 이 엔진 최초의 스폰-후
      콜라이더 재생성)과 디스크 영속화(heightmap PNG 덮어쓰기 / splatmap PNG 생성).
      리로드 후에도 편집이 유지되는지 E2E로 검증)
- [x] 데모 씬(신규 또는 기존 게임 확장)에서 실내 아레나보다 큰 야외 지형 위 이동 검증
      (sub-step 1/3 — `games/terrain-demo`, 청크 경계에 떨어뜨린 다이나믹 바디가 정확한
      높이에 착지하는 것까지 실제 프로덕션 앱 스택으로 검증)
- [x] 테스트 추가, CI 통과 (sub-step 1/3 몫 — 신규 테스트 다수, PR로 검증 중)

---

### 45. LOD (Level of Detail)
...
**목표:** 카메라에서 보이지 않는 오브젝트의 렌더링을 건너뛰어, 프러스텀 컬링만으로는
부족한 큰 씬의 draw call을 줄인다.

**완료 조건:**
- [x] 최소 하나의 오클루전 기법 구현(예: GPU occlusion query 또는 계층적 Z-버퍼)
- [x] 터레인/건물 등 오클루더가 있는 씬에서 컬링된 오브젝트 수를 프로파일러로 확인 가능
- [x] 프러스텀 컬링과 자연스럽게 합성(중복 작업 없음)
- [x] 테스트 추가, CI 통과

PR #1810 (2026-08-29)로 완료. **CPU 소프트웨어 오클루더 래스터화** 채택 — wgpu 22.1.0이
`QueryType::Occlusion`을 별도 feature 없이 지원하지만, GPU 쿼리는 비동기 readback 탓에
"지난 프레임 결과로 이번 프레임을 컬링"하는 temporal 방식이 되어 카메라가 빠르게 움직일 때
팝핑이 생기고 결정적 검증이 불가능해서 명시적으로 거절함. CPU 방식은 같은 프레임 안에서
확정되고 핵심 로직이 GPU 없이 유닛 테스트 가능.

**설계 전체가 단 하나의 규칙 위에 서 있음 — "보이는 것을 절대 컬링하지 않는다".** 놓친 컬링
기회는 약간의 성능 손해지만, 잘못된 컬링은 눈에 보이는 렌더링 버그. 이를 구조적으로 보장하는
두 메커니즘이 조합됨: (1) `Occluder` 박스는 실제 지오메트리 *안쪽*에 들어가는 보수적
과소추정이어야 함(바운딩 박스가 아님), (2) 내부 보수적 래스터화 — 픽셀 4모서리가 모두 삼각형
안에 있을 때만 기록하므로, 경계·이음매에 생기는 구멍은 언더컬링만 유발. 후보 쪽은 의도적으로
비대칭 — 바운딩 스피어를 감싸는 큐브(과대추정)로 테스트해 컬링 판정을 더 어렵게 만듦.

`bsengine-render/src/occlusion.rs`의 순수 함수 2개(`rasterize_occluder_box`/`box_occluded`,
128×128 f32 버퍼) + `Occluder { center, half_extents }` 컴포넌트. `center` 필드는 실제 소스
조사 중 발견된 필수 요소 — 터레인 청크 메쉬는 로컬 `(0,0)..(chunk_size, chunk_size)`를 차지하는
반면 `Transform`은 최소 코너에 있어 박스 오프셋이 필요함. `render_frame`은 기존 프러스텀 체크
**다음에** 오클루전 테스트를 배치해 두 컬링 단계가 중복 없이 합성됨. 터레인 청크는 로컬 y=0부터
청크 **최소** 높이까지 올라가는 박스를 자동 부착해 실제 표면 아래에 머묾. `project.toml`
`[render] occlusion_culling` 토글(리소스 부재 시 활성화, 프러스텀 컬링과 동일).

**실측 입증:** 벽 뒤 30개 + 벽 옆 3개 씬에서 켬 → occluded_count=30/draw_calls=11,
끔 → 0/71. E2E 테스트가 델타를 *정확히*(`off - on == 2 × occluded`) 단언하고, 보이는
엔티티들이 만드는 draw call 하한선을 함께 검증해 과도 컬링 시 조용히 통과하지 않고 실패함.

---

### 47. 안티에일리어싱

**목표:** 렌더링 결과의 계단 현상을 줄여 Unity/Unreal 기본 수준의 시각 품질에 근접한다.

**완료 조건:**
- [x] 최소 하나의 AA 기법 구현(MSAA 또는 TAA 중 하드웨어/파이프라인에 맞는 쪽)
- [x] 포스트프로세스 체인(블룸/SSAO)과 순서 충돌 없이 합성
- [x] 프로젝트 설정에서 켬/끔 및 품질 단계 조절 가능
- [x] 픽셀 리드백 테스트로 켬/끔 시 가장자리 픽셀 차이 검증
- [x] 테스트 추가, CI 통과

PR #1811 (2026-08-29)로 완료. **TAA 채택.** MSAA는 취향이 아니라 구체적 파이프라인 충돌 때문에
거절됨 — SSAO 셰이더가 깊이를 `texture_depth_2d`로 선언해 `textureLoad`로 직접 읽는데, MSAA를
켜면 이게 `texture_depth_multisampled_2d`가 되고 wgpu 코어 API는 컬러만 resolve하고 깊이 자동
resolve가 없음. 즉 MSAA는 SSAO 재작성이나 별도 depth prepass를 요구해 완료조건 2번과 정면 충돌.
로드맵의 "파이프라인에 맞는 쪽"이 실제로 가리킨 방향이 TAA였음.

`Taa { enabled, history_blend, clamp_strength }`를 카메라 컴포넌트로(Bloom/ToneMap/
AmbientOcclusion과 동일 패턴, `register_gameplay_reflect_types`에 등록 — windowed 전용인
RenderPlugin이 아님). **부재 시 꺼짐**이라 기존 8개 픽셀 테스트가 전부 무수정 통과. 완료조건
3번의 "프로젝트 설정"은 씬 RON에서 저작하는 카메라 컴포넌트로 해석 — 엔진의 기존 포스트프로세스
설계와 일관되고 카메라별 제어라는 이점까지 얻음.

Halton(2,3) 지터(`taa_jitter.rs`, 순수 함수 6개 유닛 테스트)를 **래스터화 프로젝션에만** 적용.
재투영 행렬은 지터를 절대 안 실음 — 지터된 행렬로 재투영하면 카메라가 아니라 지터를 쫓아가서
영원히 수렴하지 않음. composite를 중간 LDR 타겟으로 돌리는 구조 변경을 **동작 보존 커밋으로
분리**해서, 이후 픽셀 테스트가 깨지면 TAA 수학 탓인지 구조 변경 탓인지 즉시 구분 가능하게 함.
히스토리 핑퐁 2개(`TrackedTexture`라 item 43 프로파일러 메모리 집계에 잡힘)를 dual color
attachment로 스왑체인과 동시 기록. 이전 프레임 view_proj는 이 PR 전까지 엔진 어디에도 없었음.

**실측 입증:** 30도 회전한 큐브에서 전경도 배경도 아닌 중간톤 픽셀 수 — TAA 끔 **0개**(완전히
계단진 정상 상태), 켬 16프레임 후 **123개**. 짝을 이루는 `taa_leaves_the_interior_of_a_solid_
region_alone` 회귀 테스트가 이 숫자를 의미 있게 만듦 — 이게 없으면 화면 전체를 뭉갠 구현도
똑같이 통과함(item 46의 과도 컬링 하한선 단언과 같은 교훈).

**알려진 한계(로드맵 미충족 조건이 아니라 향후 품질 개선 여지):** 재투영이 깊이 기반 카메라
전용이라 정적 지오메트리+카메라 이동에는 정확하지만 움직이는 물체는 3x3 이웃 클램핑으로 억제할
뿐 완전 제거는 아님. 오브젝트별 모션 벡터는 모든 씬 파이프라인(메쉬/터레인/파티클)이 속도를 추가
렌더 타겟에 써야 하는 G버퍼 변경이라 별도 작업으로 남겨둠.

**후속 작업에 중요:** `render_frame`이 이제 Bevy 0.14의 top-level 파라미터 상한 16개를 정확히
다 씀. 다음에 이 함수를 건드리는 기능은 반드시 ParamSet 안으로 넣어야 함 — 여유가 0.

---

### 48. GI / 실시간 리플렉션 / IBL

**목표:** 직접광(방향광+포인트라이트)만 있는 현재 라이팅에 간접광과 환경 반사를 더해
전반적인 렌더링 품질을 끌어올린다.

**완료 조건:**
- [x] 최소 하나의 IBL 경로(환경 큐브맵 기반 diffuse/specular 근사) 구현
- [x] 하나 이상의 실시간 또는 베이크드 GI 근사(예: 라이트 프로브) 구현
- [x] 머티리얼의 metallic/roughness와 통합되어 반사가 표면 속성을 반영
- [x] 데모 씬에서 GI 켬/끔 스크린샷 비교로 시각적 차이 입증
- [x] 테스트 추가, CI 통과

**완료 — 2개 sub-step 모두 shipped.** [[feedback_split_when_complexity_grows]]에 따라 IBL과
라이트 프로브 GI를 분리해서 각각 PR로 진행했음.

**sub-step 2/2 (라이트 프로브 GI) 완료: PR #1813 (2026-08-31).** 위치에 따라 달라지는 **바운스
간접광** — 빨간 벽 옆 흰 바닥이 붉은 기를 띰. 이게 핵심: 스카이박스만 위치별로 샘플링하는
값싼 방식은 "위치별 환경광"이지 GI가 아니라서 거절함.

**가장 큰 미지수였던 프로브 캡처는 이미 있는 능력이었음.** `point_light_face_view_projs`
(surface.rs:1012)가 필요한 6개 view-projection을 정확히 그대로 만들고, 포인트 라이트 섀도우
패스가 이미 임의 위치에서 씬을 6레이어로 렌더하고 있었음 — 프로브 캡처는 새 능력이 아니라 그
기존 경로를 다른 타겟에 겨눈 것. 설계 전에 그 루프를 읽은 게 같은 걸 두 번 만드는 걸 막았음.

`LightProbeVolume { half_extents, resolution }`이 박스와 프로브 그리드를 정의. 프로브는 셀
중심이 아니라 **격자점**에 놓임(삼선형 보간은 셀의 8개 모서리로 정의되므로 이게 보간이 성립하는
조건). resolution 초과는 거절이 아니라 MAX_PROBES(32)로 클램프. 캡처 셰이더는 **직접광 전용** —
IBL도 프로브도 안 읽음(읽으면 프로브가 프로브를 비추는 재귀가 되고, 이건 의도적으로 1바운스).
면당 16×16으로 충분: 결과를 SH 9계수로 줄여버리므로 고해상도는 버려지는 작업.

**조용히 틀렸을 두 지점:** (1) 볼륨 안에서 프로브 irradiance는 IBL irradiance를 **대체**해야 하고
가산하면 안 됨 — 프로브가 씬을 캡처할 때 스카이박스도 담았으므로 더하면 하늘빛 이중 계산.
스페큘러는 유지(프로브는 확산 전용이라 대체할 반사가 없음). (2) 셰이더는 8개 이웃의 **계수를
먼저 섞고 한 번 평가** — 8번 평가 후 결과를 섞는 것과 동등하지만 더 쌈(SH 평가가 계수에 선형).

유니폼 버퍼 + 고정 최대치(스토리지 버퍼 아님 — 이 엔진은 스토리지 버퍼를 어디에도 안 쓰고
MAX_POINT_LIGHTS 식 고정 배열이 관례). 4608바이트, `@group(2) @binding(9)`. 베이크는 볼륨이
나타나거나 파라미터가 바뀔 때 1회.

**검증의 핵심은 밝기가 아니라 색 번짐.** 망가진 구현도 밝아지기 때문. 주 테스트는 R 채널이 G/B
**보다 더** 오르는지를 채널별로 단언함 — 붉은 편향은 G/B가 *내려가서*도 생길 수 있으므로.
`a_green_wall_bleeds_green_not_red`가 계수 인덱싱을 틀려 씬과 무관하게 같은 색으로 물들이는
구현을 잡아냄. 유닛 테스트는 조용한 실패를 겨냥: SH 정규화(균일 환경 → L0만, 재구성이 원래 상수),
삼선형 가중치 합 1.0, 격자점 정확성, 큐브면 텍셀 입체각 합이 완전한 구, 캡처 셰이더가 IBL 리소스를
정말 안 읽는지, 베이크 결정성(리플레이 재현성).
...
**목표:** 대기 산란 효과로 공간감과 라이트 샤프트 같은 분위기를 표현한다.

**완료 조건:**
- [x] 카메라 프러스텀 기반 볼류메트릭 포그 패스(예: froxel 기반) 구현
- [x] 기존 방향광/포인트라이트와 상호작용(라이트 샤프트)
- [x] 씬/스크립팅에서 밀도·색상 파라미터 조절 가능
- [x] 테스트 추가, CI 통과

**완료 — 2개 sub-step 모두 shipped.**

**sub-step 2/2 (라이트 샤프트) 완료: PR #1815 (2026-08-31).** sub-step 1/2는 모든 froxel을
밝다고 취급해서 균일 안개였음. 여기서 그림자 판정을 추가해 샤프트가 됨 — froxel에 닿기 전에
차단된 빛은 그 froxel에서 산란하지 않으므로 빛기둥 사이에 어두운 틈이 생김.

**구현을 좌우한 제약: `textureSampleCompare`는 컴퓨트 셰이더에서 못 씀**(암시적 미분이 필요해
프래그먼트 전용). 씬 셰이더의 `shadow_factor`가 그걸 쓰므로 주입 패스에 그대로 복사 불가.
컴퓨트 가능한 형태는 **`textureSampleCompareLevel`**(naga 22.1.0에 존재함을 플랜 작성 전 확인).
나머지는 씬 셰이더와 정확히 일치시킴(w 나누기, UV 매핑, 범위 밖 1.0 조기 반환, 0.003 바이어스)
— 그래야 같은 월드 위치의 froxel과 표면이 그림자 여부에 합의함.

포인트/스팟은 대체 불필요 — 기존 `point_shadow_factor`가 이미 `textureLoad`로 R32Float 큐브
배열을 수동 면 선택해 읽고(그 포맷이 필터링 불가라 강제된 설계), `textureLoad`는 컴퓨트에서
그대로 동작. **면 선택 로직은 재유도하지 않고 그대로 이식** — 주석에 "verified by hand against
glam's look_at_rh convention"이라 적혀 있고, 면을 틀리면 그럴듯해 보이면서 발견이 매우 어려움.

**포인트/스팟을 별도 태스크로 고집한 게 옳았음** — 섬도우 경로가 방향광과 완전히 다른 코드라,
방향광만 검증했으면 완료조건의 절반이 미검증인 채로 "done"처럼 보였을 것.

**실측 — 단언은 "안개가 밝다"가 아니라 "그림자가 안개를 어둡게 한다":**
| | 오클루더 없음 | 광원 경로에 오클루더 | 옆으로 치움 |
|---|---|---|---|
| 방향광 | 80.0 | **56.0** | 80.0 |
| 포인트 라이트 | 101.0 | **3.0** | 101.0 |

"옆으로 치움" 열이 그 숫자를 의미 있게 만듦 — 없으면 상수 어둡기도 통과함.
`the_occluder_is_never_drawn_in_the_frame`이 샘플이 오클루더 자체 픽셀이 아니라 안개를
측정한다는 걸 보장.

**밴딩 대응 2개를 의도적으로 별도 커밋으로 분리**(`bd02c267` 지터 디더링, `9856fe4f` 시간적
재투영 누적). 샤프트 가장자리는 고대비라 균일 안개보다 Z 밴딩이 훨씬 잘 보임. 지터가 띠를
노이즈로 바꾸고 시간적 누적이 그 노이즈를 평균해 없앰. **둘을 한 커밋에 넣으면 결과가 이상할 때
어느 쪽 탓인지 구분이 안 되므로 분리가 리스크 완화 수단이었음.** 시간적 누적은 item 47 TAA의
Halton 수열과 히스토리 핑퐁 형태를 재사용 — 새로 설계하지 않음. 볼륨 밖 히스토리는 거부하고
현재 값을 그대로 씀(TAA가 화면 밖 히스토리에 쓰는 규칙과 동일).
[[feedback_split_when_complexity_grows]]에 따라 분할. 라이트 샤프트(주입 패스에서 섬도우맵
샘플링 + 포인트/스팟 기여)가 sub-step 2/2이자 유일한 미충족 조건. 분할 이유: 처음 도입하는
컴퓨트/3D 텍스처 인프라와 섀도우 샘플링 수학을 동시에 디버깅하면, 포그가 이상할 때 어느 쪽
탓인지 구분이 안 됨.

**sub-step 1/2 완료: PR #1814 (2026-08-31).** **이 엔진 최초의 컴퓨트 파이프라인과 최초의 3D
텍스처.** 이 브랜치 전까지 `ComputePipeline`/`@compute`/`TextureDimension::D3` 전부 0건이었음.
item 48의 큐브맵과 같은 구도 — 값싼 포스트프로세스 레이마칭 대신 froxel을 택한 대가로 새 인프라
비용을 치렀지만, 엔진은 포그 전용 배관이 아니라 **재사용 가능한 컴퓨트/볼륨 텍스처 지원**을 얻음.

160×90×64 `Rgba16Float` 볼륨 2개에 컴퓨트 패스 2개: 주입(froxel별 산란/소멸 + Henyey-Greenstein
위상함수로 가중한 태양광 인스캐터링) → 적분((x,y) 열마다 Z를 전방에서 후방으로 행진하며 산란과
투과율 누적). 적용은 룩업 1회.

**Z는 지수 분포**(선형이면 원근 특성상 가까운 곳에 밴딩이 생기고 먼 곳에 froxel을 낭비). 이 매핑이
Rust/주입 WGSL/`depth_to_froxel_w` 세 곳에 나타나며 셋이 반드시 일치해야 함 — 어긋나면 포그가
잘못된 거리에 생기고 밀도 문제처럼 보임. 슬라이스 적분은 점 샘플이 아니라 **해석적**(점 샘플은
두꺼운 슬라이스를 편향시키는데, 지수 분포는 원거리에 두꺼운 슬라이스를 반드시 만듦).

**적용은 HDR 단계, bloom이 읽기 전** — 밝은 광원 주변의 짙은 안개가 bloom을 타야 분위기가 삶.
패스가 자기가 렌더하는 텍스처를 샘플링할 수 없으므로 HDR을 두 번째 타겟으로 우회시켜야 했고,
이 구조 변경은 **동작 보존 커밋으로 분리**해서 이후 픽셀 테스트 실패의 원인을 명확히 함(TAA/IBL에
이어 세 번째로 이 패턴이 효과를 봄). **배경 픽셀에도 포그 적용** — SSAO는 배경을 건너뛰지만
따라하면 짙은 안개 뒤 지평선만 선명해서 즉시 틀려 보임.

스크립팅(완료조건 3): `setFogEnabled`/`setFogDensity`/`setFogColor(cam, r, g, b)`/
`setFogAnisotropy`. 색은 의도적으로 op 하나 — `setFogColorR/G/B`로 쪼개면 catalog R2 래칫 위반.
현재 62 컴포넌트 / 262 ops, 0 위반.

**실측:** 동일한 큐브 2개(근/원)에서 포그 적용 시 색 이동이 근거리 17.0 vs **원거리 219.0,
비율 12.9배**. 핵심은 뿌예짐이 아니라 **깊이 의존성** — 화면 전체에 상수를 더한 구현도 "뿌예짐"
단언은 통과하기 때문. 반대 방향 회귀(density=0 불변, 컴포넌트 없으면 픽셀 완전 동일)와
`positive_anisotropy_brightens_the_view_toward_the_light`(위상함수가 실제로 적용된다는 증거,
아니면 균일한 회색 안개)를 함께 검증.

**조용히 실패하는 두 지점에 전용 유닛 테스트:** Henyey-Greenstein의 `1/(4π)`를 빠뜨리면 포그가
약 12배 밝아지는데 이건 "밀도 상수 튜닝이 필요하네"로 읽히지 버그로 안 보임 → 구 전체 적분이
1인지 단언. Z 슬라이스는 단조·far 평면 정확히 도달·원거리가 근거리보다 10배 이상 두꺼운지 단언.
서브에이전트가 플랜에 없던 `cube_sample_is_really_on_the_cube`도 추가함 — 이게 없으면 위 12.9배
수치가 큐브가 아니라 빈 공간을 측정한 것일 수 있었음.

---

### 50. 노드 기반 머티리얼/셰이더 그래프

**목표:** WGSL 텍스트 없이 시각적으로 셰이더 그래프를 구성할 수 있는 에디터 도구 제공.
AI 에이전트 워크플로우를 위한 WGSL 텍스트 직접 작성 경로는 대체되지 않고 계속 1급으로
유지된다 — 이 항목은 인간 아티스트 워크플로우를 위한 추가 경로다.

**완료 조건:**
- [x] 노드 기반 그래프 UI(입력/연산/출력 노드, 연결선)
- [x] 그래프를 컴파일해 기존 `CustomShader`가 쓰는 WGSL로 변환(기존 텍스트 경로와 공존,
      대체 아님)
- [x] 최소 하나의 데모용 그래프(예: 간단한 스크롤 텍스처 또는 디졸브)로 왕복 검증
- [x] 테스트 추가, CI 통과

**완료 — 2개 sub-step 모두 shipped.**

**sub-step 2/2 (노드 에디터 패널) 완료: PR #1817 (2026-09-01).** 읽기 전용 뷰어가 아니라
실제로 편집 가능한 패널 — 노드 이동, 연결선 생성/삭제, 노드 추가/삭제, RON 로드/저장,
컴파일(오류는 해당 노드 옆 인라인 표시). 기존 `EditorPanel` 트레이트 위에 올라감.

**노드는 각각 `egui::Area`가 아니라 패널 자체 좌표계에 `egui::Painter`로 직접 그림** — 나중에
팬/줌을 얹을 때 좌표 변환 한 곳만 고치면 되도록. 팬/줌 자체는 의도적으로 범위 밖(좌표 변환이
모든 히트테스트와 드래그 경로에 얽힌다고 경고했고 사용자가 그걸 알고 제외에 동의).

**`GraphNode`에 `position: [f32; 2]` 추가(`#[serde(default)]`).** 이 default는 장식이 아님 —
sub-step 1/2에서 커밋한 `scroll.shadergraph.ron`에 이 필드가 없으므로, 없으면 **이미 배포된
에셋이 파싱을 멈춤**. 전용 테스트로 고정. 레이아웃은 저작된 에셋의 일부이지 UI 전용 상태가
아님(Unity/Unreal도 그래프 자산에 저장하고, 별도 파일이면 그래프를 공유할 때 레이아웃이 사라짐).
컴파일러는 이 필드를 계속 무시함.

**연결은 수락 전에 타입 검사** — `NodeKind::input_ports()`/`output_type()`을 이번에 public으로
만들어 씀. **포트 테이블을 UI용으로 복제하지 않음**: 단일 진실 원천이라야 에디터의 수락/거부
판단이 컴파일러와 *같은 테이블*을 쓰고, 복제본이 갈라져서 UI가 만든 엣지를 컴파일러가 거부하는
일이 안 생김.

**설계를 바꾼 테스트 제약:** 이 프로젝트는 패널 테스트에서 하드코딩된 클릭 좌표를 금지함(PR
#1727에서 좌표를 손으로 세 번 다시 측정한 뒤 도입된 규칙, `panels/inspector.rs:529`에 문서화).
기존 헬퍼는 `Shape::Text` 갤리를 훑어 위젯을 찾는데, **Painter로 그린 박스와 포트 원은
`Shape::Rect`/`Shape::Circle`이라 그 헬퍼로 포트를 찾을 수 없음.** 그래서 패널이 이번 프레임에
계산한 포트 중심을 public `last_port_positions`에 기록하고, 테스트가 패널이 실제로 쓴 좌표로
상호작용을 구동함 — 규칙을 규율이 아니라 구조로 지킴.

**패널 테스트 13개, 단언은 픽셀이 아니라 모델에.** 렌더링은 훌륭하지만 아무것도 안 바꾸는
패널도 "그려진다" 체크는 통과하므로. 특히:
- `a_type_mismatched_connection_is_refused` — **타입 가드 뮤테이션 테스트.** vec3 출력을 f32
  입력에 떨어뜨리면 `edges`가 **불변**이어야 함. 없으면 "연결이 된다"만 증명하고 모든 연결을
  받아주는 구현도 똑같이 통과함. `a_connection_released_over_empty_canvas_is_cancelled`가 같은
  가드를 반대쪽에서 검증.
- `deleting_a_node_also_removes_its_edges` — 미묘한 것. 매달린 엣지를 남기면 나중에
  `UnknownNode`로 컴파일이 실패하는데, 원인이 된 삭제에서 한참 떨어진 곳에서 터짐.
- `compiling_from_the_panel_matches_calling_compile_directly` — 바이트 동일. 패널이 갈라진
  두 번째 경로가 되면 안 됨.
- `the_demo_graph_loads_and_saves_without_changing_meaning` — 직렬화 비대칭은 아티스트 파일을
  첫 저장에서 조용히 망가뜨림.

**item 50의 철학적 긴장은 분할이 회피가 아니라 해소함.** sub-step 1/2의 그래프 포맷+컴파일러는
AI 에이전트가 RON을 직접 써서 쓰는 경로이고, 이 sub-step이 로드맵이 말하는 인간 아티스트 경로.
"WGSL 텍스트 경로는 1급 유지"가 이제 산문 속 의도가 아니라 **통과하는 테스트로 고정됨**.
[[feedback_split_when_complexity_grows]]에 따라 분할. 노드 그래프 UI(드래그, 연결선,
히트테스팅, 팬/줌)와 컴파일러는 성격이 아주 다른 서브시스템이라, 나누면 셰이더가 깨졌을 때
UI가 그래프를 잘못 만든 건지 컴파일러가 잘못 번역한 건지 구분됨. egui 노드 에디터 패널이
sub-step 2/2이자 유일한 미충족 조건.

**분할의 부수 효과가 이 항목의 철학과 맞아떨어짐:** 그래프 파일 포맷 + 컴파일러는 **AI
에이전트가 RON을 직접 써서 쓸 수 있는 경로**이고, 노드 에디터는 로드맵이 말하는 인간 아티스트
경로임. 이 항목 자체가 "WGSL 텍스트 경로는 대체되지 않고 1급 유지"라고 명시하는데, 이제
그게 의도가 아니라 **테스트로 고정됨**(손으로 쓴 glow.wgsl 쓰는 기존 픽셀 테스트가 전부
무수정 통과).

**sub-step 1/2 완료: PR #1816 (2026-09-01).** 새 leaf 크레이트 `bsengine-shadergraph` —
그래프 in, WGSL 문자열 out. ECS/GPU/egui 의존성 0이라 어댑터 없이 유닛 테스트되고 나중에
에디터 UI와 에셋 툴링 양쪽에서 쓸 수 있음. 노드 12개(Uv/Time/Constant/ConstantVec3/
TextureSample/Add/Multiply/Sin/Lerp/Step/Fract/Output) — 로드맵이 명시한 데모를 만들기에
정확히 충분한 최소 집합. 위상 정렬 후 노드당 SSA `let` 한 줄 방출, **Output에서 역방향
도달 가능한 노드만** 방출해서 죽은 노드 제거가 순회에서 공짜로 따라옴(테스트로 고정해 나중에
"전부 방출"로 조용히 퇴화하지 않게 함). 오류는 패닉이 아니라 값(`Cycle`/`NoOutput`/
`MissingInput`/`TypeMismatch`) — sub-step 2/2 에디터가 반쯤 만든 그래프에 대고 계속 호출할
것이므로.

**출력은 `.wgsl` 파일**이고 `CustomShader.path`가 손으로 쓴 셰이더와 똑같이 가리킴. 다운스트림
변경 0, 그래프가 이상하게 동작할 때 생성된 WGSL을 사람이 읽을 수 있음, 그리고 "공존, 대체
아님"이 구조적으로 참이 됨.

**조용히 실패했을 두 지점(플랜 작성 전 실제 소스를 읽어서 잡음):**
1. **프리앰블이 `MESH_WGSL`과 바이트 단위로 일치해야 함.** 커스텀 셰이더는 표준 메쉬
   파이프라인과 *같은* `pipeline_layout`으로 컴파일되므로 `CameraUniform`/`ModelUniform`이
   어긋나면 wgpu 검증 내부에서 패닉하는데, 메시지가 컴파일러가 아니라 파이프라인을 가리킴.
   구조체를 그대로 방출하고 `MESH_WGSL`에서 추출해 바이트 동일성을 단언하는 전용 테스트를 둠.
2. **`glow.wgsl`의 `VertOut`에는 UV가 없음**(world_normal/world_pos만). 생성 셰이더는 정점·
   프래그먼트 *양쪽*을 방출하므로 `uv`를 포함한 자기 `VertOut`을 선언하고 `VertIn`의
   `@location(3)`에서 통과시킴. 기존 형태를 그대로 복사했으면 텍스처를 쓰는 모든 그래프가
   깨졌을 것.

**왕복 테스트가 이번 사이클의 핵심** — 컴파일러 유닛 테스트만으로는 "생성된 WGSL이 진짜 이
엔진에서 컴파일되고 그린다"를 증명 못 함(naga가 받아주는 것과 이 파이프라인 레이아웃에 맞는
것은 다른 문제이고, 깨지는 건 후자). 2색 텍스처 위 스크롤 데모 그래프
(`Uv + Time × 0.1 → Fract → TextureSample → Output`) 실측: t=0.0에서 좌 `[206,0,0]` /
우 `[0,0,206]`, t=5.0에서 좌 `[118,0,189]` / 우 `[188,0,122]` — 텍스처가 실제로 움직임.
`holding_time_still_leaves_consecutive_frames_identical`과 짝지음(없으면 프레임마다 노이즈가
있어도 스크롤 테스트가 통과함).
...
**목표:** 리지드바디를 서로 구속하는 물리 조인트 프리미티브 제공 — 래그돌/비히클의 토대.

**완료 조건:**
- [x] Rapier 조인트 타입(fixed/revolute/prismatic/spring 등) 중 최소 3종을 컴포넌트로 노출
- [x] 씬 RON + 스크립팅(`Bsengine.joint.*`)으로 두 엔티티 간 조인트 부착/해제
- [x] 데모(문 힌지 또는 흔들리는 체인)로 동작 검증
- [x] 테스트 추가, CI 통과

**완료 — 4개 태스크 모두 shipped (2026-09-01).** `Joint`/`JointKind` 컴포넌트와
`PhysicsWorld::add_joint`/`remove_joint`, 씬 RON의 이름 기반 `joint:` 필드,
`Bsengine.joint.*` 네임스페이스, 그리고 흔들리는 체인 데모. 조인트 테스트 18개.

**"배관이 아니라 입구"였음.** `PhysicsWorld`는 이미 `impulse_joint_set`을 들고 있었고
`step()`이 이미 그걸 넘기고 있었음 — **아무데서도 조인트를 만들지 않았을 뿐.** 그래서 이
항목은 스케줄링 변경이 전혀 필요 없었고, 조인트가 존재하는 순간부터 기존 `step()`이 그대로
시뮬레이션함. 노출한 3종은 fixed/revolute/spherical이며, prismatic/spring/rope/pin-slot은
각자 파라미터·데모·테스트를 데려오고 래그돌에도 필요 없어서 의도적으로 제외.

**진짜 함정은 이름 해석이었음.** `Joint.body_b`는 `Entity` — 파일에 적을 수 없는 런타임
값이라, 씬은 상대 엔티티를 **이름으로** 부르고 전부 스폰된 뒤에 해석해야 함. 이건
`EntityDescriptor.parent:`가 이미 하던 일이라, **두 번째 이름 해석 메커니즘을 만들지 않고
같은 패스·같은 이름 맵을 씀**(`spawn_scene_entities`의 pass 3). 병렬 메커니즘을 만들었다면
미묘하게 다른 규칙으로 이름을 푸는 두 경로가 생겼을 것.

**`sync_joints`는 매 프레임 재시도한다 — 삽입 반응형이 아니라.** 씬의 `Joint`가 처음
존재하는 프레임에는 양쪽 다 아직 Rapier 바디가 없기 때문(`spawn_bodies`가 같은 씬의
`RigidBody`/`Collider`에서 만듦). 한 번만 만드는 설계였다면 **씬에 저작된 조인트가 전부
조용히 사라졌을 것.** 반대쪽 절반인 despawn 정리는 즉시 증상이 없는 쪽 — 정리하지 않으면
Rapier가 아무도 가리키지 않는 바디를 계속 구속함.

**데모는 문 힌지가 아니라 체인.** 힌지 하나는 조인트가 존재한다는 것만 증명하지만, 체인은
**직렬로 놓인 조인트 4개**를 증명함 — 각 조인트의 보정이 이웃을 흔들고 솔버가 넷을 동시에
만족시켜야 하는 이 거동이 정확히 item 52 래그돌이 조립되는 방식. 체인은 수평으로 시작함:
수직으로 매달면 이미 평형이라 그냥 가만히 있고 "흔들렸다"가 관측 불가능해짐.

**실측(`joint-chain.ron`, 400프레임):** 네 조인트의 앵커 간격은 최악 프레임에서
0.030/0.019/0.020/0.003, 마지막 프레임엔 ~0.002 — 링크 간격 1.0 대비. 자유단은 출발점에서
최대 6.60 유닛 이동. 측정값은 `println!`으로 출력해 추론이 아니라 숫자로 보이게 함.

**뮤테이션 테스트가 계획에 없던 세 번째 단언을 찾아냄.** 계획은 두 절반("체인이 버텼다" +
"흔들렸다")이 서로의 맹점을 덮는다고 봤는데, `ChainAnchor`를 `Dynamic`으로 뒤집으면 **둘 다
진짜 씬보다 더 잘 통과함** — 다섯 바디가 같이 떨어지면 모든 제약이 정확히 0.0으로
만족되고, 자유단은 219 유닛을 "이동"함. 고정 바디가 실제로 안 움직인다는 단언만이 흔들린
체인과 그냥 떨어진 체인을 구별함. 나머지 두 뮤테이션은 설계대로 실패: 스케줄에서
`sync_joints` 제거(간격 219), 씬 앵커 하나의 부호 스위즐(1.03) — 후자가 잡힌 이유는 테스트가
앵커 오프셋을 **검사 대상에서 읽어오지 않고 다시 적어두기** 때문.

**카탈로그:** 컴포넌트 62 → 63(`Joint`, R1), ops 266. revolute 축은 op **하나**에 인자
셋으로 들어감 — `attachRevolute_x`/`_y`/`_z`로 쪼개는 것이 R2 래칫이 막으려는 것이고,
애초에 대각선 축 힌지를 표현할 수도 없음.

---

### 52. 래그돌

**목표:** 스켈레탈 메시를 조인트로 연결된 리지드바디 체인으로 전환해 사망/피격 시 물리
기반 애니메이션을 표현한다.
...
**목표:** 발/손 IK로 지형에 맞춘 자연스러운 포즈를, 다른 스켈레톤 간 애니메이션 리타게팅을
지원한다.

**완료 조건:**
- [x] 2본 IK(다리/팔) 솔버 — 지형 높이에 맞춰 조정되는 foot IK
- [x] 스켈레톤 본 매핑 기반 애니메이션 리타게팅(다른 본 구조의 glTF 간)
- [x] 데모(경사/계단에서 foot IK 검증)
- [x] 테스트 추가, CI 통과

**완료 (2026-09-04).** sub-step 1/2(2본 IK + foot IK)는 PR #1823, sub-step 2/2(리타게팅)는 PR #1824.

**이로써 Phase 2(items 51~54: 조인트 → 래그돌 → 비히클 → 애니메이션)가 전부 완료.**

원래 문구: IK와 리타게팅은
공유 코드가 거의 없는 별개 서브시스템이라 분할([[feedback_split_when_complexity_grows]]). IK를
먼저 둔 이유: 터레인과 `fox.glb`가 이미 있어 바로 검증 가능한 반면, 리타게팅은 **두 번째 리그가
필요한데 레포에 스킨드 캐릭터가 fox.glb 하나뿐**임.

`solve_two_bone`은 순수 함수(`bsengine-gltf/src/ik.rs`). **각도 델타를 합성하지 않고 해를 직접
구성한 뒤 회전을 읽어냄** — 델타 방식은 유도된 굽힘 축에 대한 부호 규약 3개가 맞아떨어져야 하고,
첫 시도는 도달 가능한 타깃에서 1.44 m 빗나갔음. 무릎 위치를 법선 코사인으로 놓고 거기서 회전을
읽으면 도달 가능한 타깃은 **구성상** 맞음. 굽힘 평면은 현재 포즈에서 유도하므로 애니메이션의 무릎
방향이 보존되고 체인별로 저작할 게 없음. **`acos` 클램프가 핵심**: 완전 신전을 넘으면 인자가 1을
초과해 NaN이 되고, 그게 조인트 행렬을 타고 모든 스킨 정점에 퍼져 캐릭터가 통째로 사라짐 — 솔버를
전혀 가리키지 않는 증상.

`IkChains { chains: Vec<IkChain> }`가 컴포넌트(`IkChain`은 원소 타입) — 엔티티는 같은 컴포넌트를
하나만 가지므로 `Vehicle.wheels`와 같은 모양. 스키닝의 globals 누적과 inverse-bind 사이에 적용하고
**globals를 두 번 누적**함: 풀 대상 월드 위치를 얻기 위해 한 번, 푼 회전을 locals에 쓴 뒤 tip과
그 아래 본들이 따라오게 한 번. 지면 프로브 `FootIkGround`는 `bsengine-physics`에 있음 —
레이캐스트가 `PhysicsWorld`를 필요로 하고 의존 방향이 physics → gltf라, 발 위치는 반대로
`SkinnedMesh.ik_tip_positions`로 발행됨(`pose_override`와 같은 채널).

**유닛 테스트 11개가 통과하는 동안 살아 있던 결함 3개 — 각각 다른 사각지대:**
1. **좌표계.** 솔버는 모델 공간, `IkChain.target`은 월드 공간인데 변환이 없었음. 실제 여우
   (스케일 0.02)에서 발이 y=15로 보고되고 지면은 y=0. **모든 픽스처가 identity 트랜스폼이라 두
   공간이 일치했고**, 어떤 유닛 테스트도 구분할 수 없었음.
2. **엔티티당 컴포넌트 1개.** `IkChain`이 Component였고 여우는 다리가 넷. 순수 함수 테스트
   5개가 통과한 이유는 슬라이스를 직접 넘겨 체인 N개가 자연히 통과했기 때문 — **프로덕션이 쓰는
   ECS 경로를 안 탔음.**
3. **헛통과.** 계단 테스트가 "발 높이 spread > 0.1 m"만 단언해 4.04 m로 통과했는데, 타깃 셋이
   0으로 남은 상태였음 — 기능이 아니라 버그를 측정. **파생 통계는 입력 유효성부터 확인할 것.**

**로컬에서 원리적으로 못 잡는 CI 실패 하나:** `every_committed_sidecar_matches_the_asset_beside_it`
가 4개 잡 전부 실패했는데 로컬 워크스페이스는 통과. `.gitattributes`가 `.ron`을 `eol=lf`로
못박는데 `.meta`를 CRLF 워킹 카피에서 재생성해, CI의 신선한 체크아웃이 82바이트(=줄 수) 짧았음.
**워킹 트리는 사이드카가 기술한 그 파일을 갖고 있어 스스로와 일치하므로 로컬 실행은 무력.**
텍스트 에셋 편집 후엔 LF 정규화를 먼저 하고 `.meta`를 재생성할 것.

**임계값이 아니라 기하를 고칠 것.** 데모 계단이 처음 0.25 m였는데 스케일 0.02 여우의 다리가
~0.3 m라, 앞발이 위 트레드에 고정되면 몸이 들려 뒷다리가 아래 트레드에 0.228 m 못 미쳤음(경계
0.25 — 단독 통과, 전체 실패). 솔버는 문서대로 뻗은 것이라 정상이었고 데모가 못 넘을 단차를
요구한 것. 0.15 m로 낮춘 뒤 뒷발 0.044 m, 앞발 1e-7.

69 컴포넌트 / 269 ops, 0 위반. E2E 리플레이는 이제 10개.

**sub-step 2/2(리타게팅) 완료: PR #1824 (2026-09-04).** `retarget_locals`가 각 리그의 **rest
포즈 대비 델타**를 옮김(`delta = source_rest⁻¹ * source_animated`, `target_local = target_rest *
delta`). 절대 로컬 회전 복사는 명백한 구현이면서 두 리그의 바인드 포즈가 다르면 항상 틀림.
**회전만** 전송 — 본 길이는 타깃 리그의 것이고, 덮어쓰면 리타게팅이 아니라 소스 체형으로
변형하는 것. 그 결과 생기는 미끄러짐은 sub-step 1/2의 foot IK가 담당하며, 둘이 조합되는 게
의도된 설계. 본 매핑은 명시적 이름 쌍(item 52가 휴리스틱을 거절한 것과 같은 이유 — 리깅 규칙에
의존하고 다른 리그·비영어 이름에서 조용히 실패하는데, 리타게팅에서는 그게 "그럴듯한 잘못된
포즈"로 나타남). 70 컴포넌트 / 269 ops.

**리타게팅은 IK보다 먼저 실행됨.** 리타게팅이 포즈를 정하고 IK가 그 포즈를 월드에 맞춰 보정함.
순서가 뒤집히면 리타게팅이 보정을 덮어써 발 배치가 조용히 죽는데 상류는 전부 멀쩡해 보임.
뮤테이션 검증: IK 루프 뒤로 옮기면 발이 타깃에서 1.2e-7 m가 아니라 **1.83 m** 벗어남. 그 테스트는
두 기능이 **같은 limb**을 건드리게 구성해야 함 — 다른 limb이면 순서 역전이 관측되지 않음.

**⚠️ 픽스처가 성질을 관측 불가능하게 만든 사례 2건, 둘 다 뮤테이션으로만 드러남:**
1. **소스 rest가 항등이면** `source_rest⁻¹`도 항등이라 rest-델타와 절대 복사가 **수치적으로 동일**.
   단언은 옳았는데 픽스처가 잴 수 없게 만듦. 두 리그 모두 항등이 아니게 바인드해야 구분됨.
2. **두 캐릭터가 같은 클립을 재생하면** 매핑 안 된 본이 우연히 일치해서 "제 포즈를 지켰다"와
   "통째로 복사됐다"가 구분 불가. 서로 다른 클립을 줘야 함.
   그 과정에서 제 추론도 틀렸음: "여우는 IK가 도니 발 본이 다를 것"이라 봤으나, **IK는 체인의
   root/mid 로컬만 쓰고 tip은 조상이 움직여 이동하므로 tip의 로컬은 안 바뀜.**

**부수 발견 — `GltfPlugin`이 씬이 저작한 `AnimationPlayer`를 덮어씀.** glTF 로드 완료 시
`AnimationPlayer::new(first_clip_name)`를 무조건 삽입해서, 스킨드 캐릭터의 초기 클립을 씬에서
지정할 수 없음(로드 타이밍에 따라 살아남거나 지워짐). 바로 두 줄 아래의 `Transform`은
`if existing_transform.is_none()` 가드로 씬 저작값을 존중하므로, 같은 가드가 빠진 것.
**별도 PR로 수정 예정.**
...
**목표:** 프로젝트를 독립 실행 가능한 빌드로 내보낸다(최소 데스크톱 타깃).

**완료 조건:**
- [x] 에셋 쿠킹(사용 에셋만 수집, 참조 무결성 검증) — PR #1826
- [ ] 단일 실행 파일 + 에셋 번들 출력(최소 Windows/현재 개발 플랫폼)
      — **에셋 번들은 완료(PR #1827, `game.pak`), "단일 실행 파일"은 미충족.** pak 모드
      출력은 `exe + project.toml + game.pak` 3개 파일이다. 문자 그대로의 단일 파일은
      Godot이 `.pck`를 하듯 아카이브를 exe에 임베드해야 하며, 설계 때 선택지로 제시했으나
      이번에는 채택하지 않았다. 채우려면 그 경로다.
- [x] 빌드된 실행 파일이 에디터 없이 게임을 실행함을 검증 — PR #1826
- [x] 테스트 추가(쿠킹 로직), CI에서 빌드 스모크 테스트 — PR #1826
- [x] 문서화(빌드 명령어) — PR #1826

**sub-step 1/2 완료(PR #1826).** `bsengine-runtime --package <dir> [--out <dir>]`.
씬·프리팹·스크립트가 지명한 씬을 재귀 순회해 도달 가능한 에셋만 수집하고, 해석되지
않는 씬 참조는 빌드를 실패시킨다. 스크립트 안의 따옴표 경로는 추측이므로 경고만
하되(`fixup`과 같은 규칙), 해석되면 여느 참조처럼 순회한다. 정적 분석이 볼 수 없는
경로와 참조되지 않지만 필요한 파일(CC-BY 라이선스 고지 등)은 `[package] extra_assets`로
명시한다. 각 에셋은 `.meta` 사이드카와 함께 나간다 — 엔진이 그것으로 `AssetIndex`를
만들기 때문에 빼면 원본과 참조 해석이 달라진다.

**sub-step 2/2 완료(PR #1827).** `--mode pak` / `[package] mode = "pak"`이 `assets/`
대신 `game.pak` 하나를 쓴다. 포맷은 자체 컨테이너(매직 + 버전 + `(경로, 오프셋, 길이)`
인덱스 + 무압축 블롭) — Unreal/Unity/Godot이 zip 대신 자체 컨테이너를 쓰는 이유가
랜덤 액세스이고, 압축은 엔트리 단위여야 오프셋 읽기가 살아남기 때문에 무압축으로
시작해 엔트리 단위 압축을 나중에 덧붙일 수 있는 구조로 뒀다.

읽기는 두 갈래다. `PakAssetPlugin`이 아카이브를 bevy의 **기본** `AssetSource`로
등록하므로 `AssetServer::load` 호출 지점은 하나도 안 바뀐다(`init_default_source`가
`get_or_insert_with`라 `AssetPlugin` **앞에서** 등록하면 살아남는다 — 뒤에 등록하면
조용히 무시된다). 씬 RON 3곳(`bsengine-scene`의 진입 씬·프리팹,
`bsengine-runtime`의 런타임 씬 로드)은 `AssetReader`가 닿지 않아 `pak_source` shim이
맡는다. 씬을 bevy_asset으로 옮기지 않은 이유는 스폰이 비동기가 되면서 E2E 녹화 10개가
의존하는 프레임 타이밍이 바뀌기 때문이다.

**실행이 설계를 두 번 고쳤고 둘 다 유닛 테스트가 전부 초록인 상태였다:**
① `GltfSourceLoader`가 자기 `reader`를 무시하고 디스크 경로를 직접 다시 읽고 있었다 —
"bevy_asset을 거치면 커스텀 리더로 커버된다"는 결론이 틀렸던 지점. `.glb`는 리더
바이트로 디코드하고, `.gltf`는 형제 파일을 디스크에서 해석하므로 **cook이 pak 모드에서
거부**한다(런타임 에셋 로드 실패는 WARN이라 조용히 메시를 잃는다).
② `archive_key` 규칙이 두 벌 있었고 `pak_source` 쪽이 `./`만 떼서 **절대 프로젝트
경로가 전부 아카이브를 빗나가 디스크로 폴백**했다. 유닛 픽스처가 전부 `.`을 썼는데
그게 틀린 규칙과 맞는 규칙이 일치하는 유일한 값이라 E2E만 볼 수 있었다.

**부수 발견(미해결, 별도 PR 감):** `fixup`이 못 보는 참조가 두 종류 있다. ① 씬은 reflected
컴포넌트를 `(타입명, RON 소스 문자열)` 쌍으로 저장하므로 `Terrain.heightmap_path` 같은
경로가 문자열 안에 중첩되어 있고, `fixup`은 이를 리네임에 맞춰 고쳐 쓴 적이 없다.
② `games/net-2p-demo/{client,server}`는 `assets/` 디렉터리 자체가 없고 파일을 맨 이름으로
지명하는데, `scan`은 `<project_dir>/assets`를 걷고 `is_project_relative`는 `assets/…`만
인정한다. cook은 **읽는 쪽만** 우회했다 — 쓰는 쪽은 사용자 씬 파일을 편집하는 도구라
별도 작업.
...
**목표:** item 7의 기초 동기화(서버/클라이언트 역할, Transform 동기화)에 클라이언트 예측,
스냅샷 보간, 관심영역(AOI)을 추가한다.

**완료 조건:**
- [x] 클라이언트 사이드 예측 + 서버 재조정(reconciliation) — PR #1829
- [x] 다른 엔티티 스냅샷 보간(끊김 없는 이동) — PR #1828
- [x] 관심영역 기반 동기화 범위 제한(거리 기반) — PR #1828
- [x] 지연/패킷 손실 시뮬레이션 하에서 데모 검증 — PR #1828
- [x] 테스트 추가, CI 통과 — PR #1828

**⚠️ 목표 문구가 첫 조건에 대해 부정확하다.** "item 7의 기초 동기화에 추가한다"고 적혀
있으나, 현재 모델은 **클라이언트 권위**다(`NetworkRole::Client` 문서가 "sends only its
own client-authoritative transforms"라고 명시). 클라이언트가 자기 트랜스폼의 권위자면
서버가 반대할 일이 없어 **재조정할 대상 자체가 없다.** 예측은 권위 모델을 뒤집어야
하므로 sub-step 2/2로 분리했고, 사용자가 **엔티티별 옵인**을 선택해 기존 모델은 그대로
동작한다.

**sub-step 1/2 완료(PR #1828).** `MSG_TRANSFORM_BATCH`에 `u32` 서버 틱 추가(스냅샷을
시간축에 놓을 수 없으면 보간이 불가능), 클라이언트가 스냅샷을 버퍼링하고
`interpolate_remote_entities`가 설정된 지연만큼 뒤에서 렌더, 서버가 피어별로 거리 필터를
건 배치를 전송, 그리고 시드 기반 결정론적 지연/손실 시뮬레이터. 모든 `[network]` 기본값이
이전 동작을 그대로 재현한다.

**시뮬레이터는 기능이 아니라 계측기다.** 완벽한 링크에서는 보간된 위치와 최신 스냅샷이
매 프레임 일치하므로, **아무 일도 하지 않는 보간기가 손실 없는 테스트를 전부 통과한다.**
그래서 보간보다 먼저 만들었고, 시드를 고정했다(이걸 의존하는 테스트가 flaky해지면
진짜 회귀와 나쁜 주사위가 같은 빨강이 된다).

**⚠️ 뮤테이션이 알려준 것 — 테스트 하나로는 아무것도 증명 못 했다.** 보간을 "최신 스냅샷
그냥 적용"으로 바꿨을 때 `a_remote_entity_tracks_the_server_under_packet_loss`는
**통과**했다. "서버를 잘 따라간다"는 보간과 단순 적용을 구분하지 못한다. 오직
`the_client_advances_on_frames_where_nothing_arrived`만 실패한다.

**한계(숨기지 않고 문서화):** 원격 엔티티는 의도적으로 과거를 렌더링하고, 최신 스냅샷을
지나면 외삽하지 않고 **정지**한다(빗나간 추측은 진실이 도착할 때 뒤로 튄다). AOI는
엔티티를 숨기는 게 아니라 갱신을 멈춘다 — 반경 밖 엔티티는 마지막으로 본 자리에 남는다.

**sub-step 2/2 (예측+재조정) 완료(PR #1829, squash `c2c855aa`) — item 56 전체 완료.**
`NetworkAuthority::Predicted { peer_id }` 엔티티는 소유 피어가 **입력**을 보내고
서버가 시뮬레이션한다. `MSG_CLIENT_TRANSFORM`은 예측 엔티티에 대해 부족한 게 아니라
**틀린** 메시지다 — 클라이언트가 트랜스폼을 보내면 다시 권위자가 되어 재조정할 대상이
사라진다. 서버는 배치에 피어별 "마지막 적용 시퀀스"를 실어 보내고, 클라이언트는 그
이후의 미확인 입력을 권위 상태 위에 재생한다.

**서버가 같은 이동 스크립트를 돌린다**(공유 시뮬레이션). `isKeyPressed`가 엔티티별로
해석되며, 원격 입력이 없으면 로컬 키보드로 폴백한다 — 그 폴백이 기존 싱글플레이
스크립트를 전부 살린다.

**⚠️ 결함 세 개를 서로 다른 메커니즘이 잡았고, 이게 이 작업의 핵심 교훈이다:**
① **재생이 합성되지 않았다** — 테스트가 잡음. 한 V8 대여 안에서 3단계를 돌리면 셋 다
프레임 시작 위치를 읽고 마지막 쓰기만 남아 1칸만 간다(스크립트↔월드 교환이 설계상
프레임당 1회 지연 처리). **정확한 값 단언**이 잡았고 "움직였나"였으면 통과했다.
② **보정이 발행되지 않았다** — **clippy의 미사용 변수 경고**가 잡음. 스크립팅 쪽
테스트 4개는 `ReplayRequest`를 직접 주입하므로 전부 통과 중이었다. **끊어진 생산자는
소비자 쪽에서 정상과 구분되지 않는다**(item 52의 교훈을 다시 밟음). 이제 생산자 쪽
단언 2개가 있고 발행을 끄면 둘 다 실패한다.
③ **`net-2p-demo`는 실행된 적이 없었다** — 실제 씬을 **스폰하는** 테스트가 잡음. 씬이
여러 리팩터 전 스키마라 `ScenePlugin`이 파싱에 패닉했고, 그래서 `NetworkId`가 없어
복제가 전혀 안 되던 것도 아무도 몰랐다. 셋 다 고쳤고 입력 구동 예측 플레이어를 추가했다.
⚠️ **`--package`는 이 깨진 씬들에서 계속 통과했다** — cook은 타입 없는 RON으로,
`ScenePlugin`은 타입드 스키마로 파싱한다. **패키징은 참조를 검증하지 스키마를 검증하지
않는다.**

**한계(문서화됨):** 재생은 미확인 입력 수만큼 스크립트를 다시 돌린다(지연에 비례, 씬
크기 아님). 재생은 **위치만** 다시 유도하고 나머지 부수효과는 버린다 — 원래 입력 때 난
소리는 이미 났고 보정마다 다시 울리면 플레이어가 자기 지연을 듣게 된다. 랙 보상은 미구현.
...
- 필요한 것: `MSG_CLIENT_INPUT`(시퀀스 번호 포함, `MSG_CLIENT_TRANSFORM`은 예측
  ...
  클라이언트의 미확인 입력 재생.
- ⚠️ **입력으로 움직이는 데모가 필요하다.** `net-2p-demo`는 두 플레이어 모두 시계 기반
  원운동(`t += deltaTime` → 코사인)이라 **예측할 대상이 없다.**
...
기존 "보류 백로그"는 "데모 규모에 필요한가"를 기준으로 미뤄뒀던 목록이었으나, 이 기준은
틀렸다 — [BSENGINE_VS_UNITY_UNREAL.md](docs/BSENGINE_VS_UNITY_UNREAL.md) 비교 로드맵을
만든 이유 자체가 "지금 필요한가"가 아니라 "Unity/Unreal 대비 격차를 줄이는 것"이기 때문이다
(2026-08-27, 사용자 지적으로 재검토). 위 15개 항목은 그 비교 문서의 종합 판정표 축 등급을
기준으로 재우선순위화됐다: 유일하게 남은 "큼" 등급 축인 렌더링(콘텐츠 도구)의 7개 항목
(43~50)을 먼저 끝내고, 이미 "작음"/"중간"으로 줄어든 축의 잔여 항목(51~54), 판정표에 없는
독립 축(55~57) 순으로 배치했다. 각 항목의 상세 기술 설계는 착수 시점에 별도로 진행한다.
근거 전체는 `docs/superpowers/specs/2026-08-27-roadmap-backlog-reprioritization-design.md`
참조.
