...
- [ ] **(D)** 옛 경로 복구 + `fixup` 명령/MCP 툴
- [ ] 에셋을 리네임한 뒤 참조가 복구되는지 보는 E2E 1개 (유닛 테스트로는 안 덮이는 부분)
- [ ] 테스트 추가, CI 통과
...
**완료 조건:**
- [ ] `ScriptSource` 에셋 타입 + 로더, `load_scripts`가 `bsengine_asset::load_async` 경유
- [ ] item 24 Phase 2가 세운 request-once/retain-handle 불변식 준수 — 핸들을 버리면
  `AssetEvent::Modified`가 안 오므로 핫리로드가 아무 증상 없이 죽는다
- [ ] 스크립트 파일을 저장하면 실행 중인 게임에 반영 (Phase 3a의 재구축 기계가 그대로 적용)
- [ ] `RELOADABLE_EXTENSIONS`에 `js` 추가 — 이때 item 30의 `IDENTIFIED_EXTENSIONS`와 다시
  갈라진다(둘이 다른 질문에 답하므로 합치지 않는다는 판단의 실제 사례)
- [ ] E2E 8개가 전부 통과 (전부 스크립트 구동이므로 이게 실질 검증선)
- [ ] 테스트 추가, CI 통과
