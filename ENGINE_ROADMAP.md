...
**3개 하위 스텝으로 분리해 순차 진행 중** (각각 독립된 브레인스토밍→spec→plan→구현
사이클): (1) 터레인 코어(청크+콜라이더+데모 검증) — 완료, (2) 텍스처 스플래팅 — 완료,
(3) 에디터 브러시 툴. 전부 완료되기 전까지 이 항목의 완료 조건 체크박스는 부분만 체크된
상태로 둔다.
...
- [ ] 에디터에서 높이맵 임포트 + 브러시로 높이/텍스처 편집 가능 (sub-step 3/3 — 높이맵
      임포트 자체는 sub-step 1/3에서 이미 됨, 브러시 편집만 남음)
