...
- [ ] 단일 실행 파일 + 에셋 번들 출력(최소 Windows/현재 개발 플랫폼)
      — 현재 출력은 `exe + assets/` **디렉터리**(단일 파일 아님). 단일 파일화는 sub-step 2/2
...
**sub-step 2/2 남음:** pak 아카이브 모드 + 실제 엔진들처럼 패키징 방식을 고르는
프로젝트 설정. 플래그 하나가 아니라 별개 작업인 것이 소스로 확인됨 — 읽기 경로가 둘로
갈린다. bevy_asset 경로(glTF·텍스처·오디오)는 커스텀 `AssetReader`를 `AssetSource`로
등록하는 설계된 확장점이 있지만, 씬 RON(`bsengine-scene`)·스크립트
(`bsengine-scripting`)·`project.toml`은 bevy_asset을 거치지 않고 `std::fs`로 직접 읽는다.
