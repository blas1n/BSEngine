...
**완료 조건:**
- [ ] 볼록 사각형 분해 + 인접 그래프 (순수 Rust)
- [ ] 폴리곤 A* + 퍼널 직선화 — 그리드 경로보다 짧고 계단이 없음을 테스트로 고정
- [ ] `NavMesh::new`/`set_walkable`이 폴리곤 저작 경로로 계속 동작 (스크립팅 표면 무변경)
- [ ] 씬의 충돌 지오메트리로부터 빌드
- [ ] 그리드로는 못 하던 사례로 검증 — 좁은 통로, 비직사각형 레벨. **경사로는 범위 밖**
- [ ] `games/mini-arena`의 Enemy 추적이 새 구현으로도 동작 (E2E)
- [ ] 테스트 추가, CI 통과
