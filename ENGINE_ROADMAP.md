...
**완료 조건:**
- [ ] `Terrain` 컴포넌트 — 높이맵 데이터 + 청크 단위 메시 생성
- [ ] 최소 하나의 텍스처 스플래팅(레이어 블렌딩) 지원
- [ ] Rapier 충돌체 자동 생성(하이트필드 콜라이더)
- [ ] 에디터에서 높이맵 임포트 + 브러시로 높이/텍스처 편집 가능
- [ ] 데모 씬(신규 또는 기존 게임 확장)에서 실내 아레나보다 큰 야외 지형 위 이동 검증
- [ ] 테스트 추가, CI 통과
