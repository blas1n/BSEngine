...
**완료 조건:**
- [ ] **(A)** 사이드카 + 스캔 + 인덱스 + 고아 복구 (소비자 없이 단독 검증)
- [ ] **(B)** 씬 RON에 guid 필드, 해석 순서, 기존 씬 10개 마이그레이션
- [ ] **(C)** 스크립트를 에셋으로 승격 (+ 스크립트 핫리로드) — **자체 항목 후보.**
  Phase 2가 네 소비자에 한 동기→비동기 전환과 같은 종류이고, E2E 8개가 검증선이다.
  A·B가 끝나 실제 비용이 보일 때 분리 여부를 정한다
