...
**목표:** "요청은 한 번, 폴링만, 실패는 종착"이 다섯 곳에 각자 구현돼 있는 것을 한 타입으로.
...
**설계의 핵심은 경계다.** 슬롯이 **상태 기계**를, 호출자가 **키잉**을 소유한다 — 다섯이
달랐던 것은 대부분 키(맵·엔티티 컴포넌트·전역 슬롯 하나)였다. 그리고 셰이더의
`CompileFailed`는 `Unusable`로 일반화된다: **로드됨과 쓸 수 있음은 다르고**, 두 실패의
재시도 조건이 다르다.

**완료 조건:**
- [ ] `bsengine-asset`에 `AssetSlot<A, R>` — Loading / Ready{value} / Unusable / GaveUp
- [ ] 다섯 사이트 이전, 각 이전마다 **그 사이트의 기존 테스트가 초록**
- [ ] 슬롯 자체 테스트 5개, 각각 변이 검증
- [ ] E2E 8개·픽셀 23개 회귀 없음, CI 통과

**비용을 미리 적어 둔다.** `PendingGltf`는 지금 상태 enum이 없다(컴포넌트의 존재가 곧
"요청함"). 통합하면 쓰지 않는 상태를 갖는다. 그리고 스카이박스의 `Ready`는 현재 **디코드**를
뜻하는데 슬롯에서는 **업로드**를 뜻하게 된다 — 결과는 같지만 의미가 바뀌므로, 그 사이트의
테스트를 고칠 때 "이름에 맞추는 것"과 "동작 변화를 덮는 것"을 구분해야 한다.
