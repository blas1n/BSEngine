...
**완료 조건:**
- [ ] CPU 프레임 타임 + wgpu 패스별(섀도우/메인/블룸/SSAO/파티클/egui) GPU 타임스탬프
      쿼리 수집
- [ ] 에디터에 프로파일러 패널 추가 — 프레임 그래프 + 패스별 막대 그래프
- [ ] draw call/삼각형 수/텍스처 메모리 등 기본 통계 노출
- [ ] 헤드리스 테스트 모드에서도 통계를 조회 가능(MCP `test_query_state`로 회귀 감지에
      활용 가능하도록)
- [ ] 테스트 추가, CI 통과
