...
namespace BE::Math
{
	...
	Vector2 Lerp(const Vector2& a, const Vector2& b, const float delta) noexcept
	{
		return a + delta * (b - a);
	}

	Vector3 Lerp(const Vector3& a, const Vector3& b, const float delta) noexcept
	{
		return a + delta * (b - a);
	}

	Vector4 Lerp(const Vector4& a, const Vector4& b, const float delta) noexcept
	{
		return a + delta * (b - a);
	}
}