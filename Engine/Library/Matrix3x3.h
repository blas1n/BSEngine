...
namespace BE::Math
{
	...
	public:
		...
		explicit Matrix3x3(const float elems[3][3]) noexcept
			: mat{ elems } {}
		...
		friend inline Matrix3x3 operator+(const Matrix3x3& lhs, const Matrix3x3& rhs) noexcept
		{
			auto ret = lhs;
			return ret += rhs;
		}
		...
		friend inline Matrix3x3 operator*(const Matrix3x3& mat, const float scaler) noexcept
		{
			auto ret = mat;
			return ret *= scaler;
		}

		friend inline Matrix3x3 operator*(const float scaler, const Matrix3x3& mat) noexcept
		{
			auto ret = mat;
			return ret *= scaler;
		}

		friend inline Matrix3x3 operator/(const Matrix3x3& mat, float scaler) noexcept
		{
			auto ret = mat;
			return ret /= scaler;
		}
