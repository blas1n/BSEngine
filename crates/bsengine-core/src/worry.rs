...
impl Worry {
    pub fn fret(&mut self, amount: f32) {
        ...
        if self.anxiety >= self.max_anxiety && prev < self.max_anxiety {
            self.just_anxious = true;
        }
    }

    pub fn calm(&mut self, amount: f32) {
        if !self.enabled || self.anxiety <= 0.0 {
            return;
        }
        self.just_anxious = false;
        self.just_calm = false;
        ...
        if self.anxiety <= 0.0 && prev > 0.0 {
            self.just_calm = true;
        }
    }

    pub fn tick(&mut self, dt: f32) {
        if !self.enabled || self.anxiety >= self.max_anxiety {
            return;
        }
    ...
    pub fn is_anxious(&self) -> bool {
        self.enabled && self.anxiety >= self.max_anxiety
    }
    ...
    pub fn anxiety_fraction(&self) -> f32 {
        if self.max_anxiety <= 0.0 {
            return 0.0;
        }
        self.anxiety / self.max_anxiety
    }

    pub fn effective_dread(&self, scale: f32) -> f32 {
        self.anxiety_fraction() * scale
    }
}
...
mod tests {
    ...
    #[test]
    fn default_anxiety_zero() {
        assert_eq!(Worry::default().anxiety, 0.0);
    }
    #[test]
    fn default_enabled() {
    ...
    fn tick_increases() {
        let mut w = inst();
        w.tick(1.0);
        assert_eq!(w.anxiety, 10.0);
    }
    #[test]
    fn tick_scales_with_dt() {
        let mut w = inst();
        w.tick(2.0);
        assert_eq!(w.anxiety, 20.0);
    }
    #[test]
    ...
    #[test]
    fn is_anxious_true_at_max() {
        let mut w = inst();
        w.anxiety = 100.0;
        assert!(w.is_anxious());
    }
    #[test]
    fn is_anxious_false_below_max() {
        let mut w = inst();
        w.anxiety = 50.0;
        assert!(!w.is_anxious());
    }
    #[test]
    fn is_anxious_false_when_disabled() {
        let mut w = inst();
        w.anxiety = 100.0;
        w.enabled = false;
        assert!(!w.is_anxious());
    ...
    #[test]
    fn anxiety_fraction_zero_when_zero() {
        let w = inst();
        assert_eq!(w.anxiety_fraction(), 0.0);
    }
    #[test]
    fn anxiety_fraction_one_at_max() {
        let mut w = inst();
        w.anxiety = 100.0;
        assert_eq!(w.anxiety_fraction(), 1.0);
    }
    #[test]
    fn anxiety_fraction_half_at_midpoint() {
        let mut w = inst();
        w.anxiety = 50.0;
        assert_eq!(w.anxiety_fraction(), 0.5);
    }
    #[test]
    fn anxiety_fraction_zero_when_max_zero() {
        let mut w = inst();
        w.max_anxiety = 0.0;
        assert_eq!(w.anxiety_fraction(), 0.0);
    }
    #[test]
    fn effective_dread_scales() {
        let mut w = inst();
        w.anxiety = 50.0;
        assert_eq!(w.effective_dread(2.0), 1.0);
    }
    #[test]
    fn effective_dread_zero_when_zero() {
        let w = inst();
