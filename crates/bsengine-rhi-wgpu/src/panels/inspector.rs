...
fn draw_add_component(
    ...
    if button_response.clicked() {
        ...
        if !was_open {
            ui.memory_mut(|m| {
                ...
                // immediately. Only a request is recorded here: the
                // TextEdit's id is generated by `Ui::add` inside the popup
                // closure below and is not known at this point, so
                // `request_focus` cannot be called from here.
    ...
    egui::popup::popup_above_or_below_widget(
        ...
        |ui| {
            ...
            // Consume the request the button's open-click recorded. Done
            // here because `request_focus` needs the id `add` just
            // generated; the flag is the only way to carry the intent
            // across from the click site.
...
mod tests {
    ...
    fn picker_search_field_is_focused_on_open() {
        ...
        assert!(
            filtered.iter().any(|text| text == "PointLight"),
            "the typed text must reach the search field without clicking it, got: {filtered:?}"
        );
        assert!(
            !filtered.iter().any(|text| text == "Camera"),
            "a non-matching type must be filtered out, got: {filtered:?}"
