...
///
/// A real `.js` file rather than a string literal: it is 669 lines, and inside
/// `ops.rs` it had no syntax highlighting, no diff worth reading, and no way to
/// be linted. Nothing else in this crate ships JS, so this is the first one.
pub const BOOTSTRAP_JS: &str = include_str!("js/prelude.js");
