...
fn run_scripts(world: &mut World) {
    ...
    for cmd in commands {
        match cmd {
            ...
            ScriptCommand::CircleVulture { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut vul) = world.get_mut::<bsengine_core::Vulture>(e) {
                        vul.circle(amount);
                    }
                }
            }
            ScriptCommand::ScatterVulture { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetVultureEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::EarnWage { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Wage>(e) {
                        w.earn(amount);
                    }
                }
            }
            ScriptCommand::SpendWage { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWageEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::BetWager { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Wager>(e) {
                        w.bet(amount);
                    }
                }
            }
            ScriptCommand::ForfeitWager { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWagerEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::LamentWail { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Wail>(e) {
                        w.lament(amount);
                    }
                }
            }
            ScriptCommand::ConsoleWail { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWailEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::RouseWake { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Wake>(e) {
                        w.rouse(amount);
                    }
                }
            }
            ScriptCommand::LullWake { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWakeEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::StepWalk { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Walk>(e) {
                        w.step(amount);
                    }
                }
            }
            ScriptCommand::RestWalk { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWalkEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWallRunSpeed { name, value } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Wall>(e) {
                        w.run_speed = value;
                    }
                }
            }
            ScriptCommand::SetWallSlideSpeed { name, value } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWallJumpForce { name, value } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Wall>(e) {
                        w.wall_jump_force = value;
                    }
                }
            }
            ScriptCommand::SetWallEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::StepWaltz { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Waltz>(e) {
                        w.step(amount);
                    }
                }
            }
            ScriptCommand::HaltWaltz { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWaltzEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::ChannelWand { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Wand>(e) {
                        w.channel(amount);
                    }
                }
            }
            ScriptCommand::DischargeWand { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWandEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::StartWane { name, duration } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Wane>(e) {
                        w.start(duration);
                    }
                }
            }
            ScriptCommand::StopWane { name } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWaneEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::ManeuverWangle { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Wangle>(e) {
                        w.maneuver(amount);
                    }
                }
            }
            ScriptCommand::FoilWangle { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWangleEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::CraveWant { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Want>(e) {
                        w.crave(amount);
                    }
                }
            }
            ScriptCommand::SateWant { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWantEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::IndulgeWanton { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Wanton>(e) {
                        w.indulge(amount);
                    }
                }
            }
            ScriptCommand::RestrainWanton { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWantonEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::ReinforceWard { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Ward>(e) {
                        w.reinforce(amount);
                    }
                }
            }
            ScriptCommand::AbsorbWard { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWardEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::KindleWarm { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Warm>(e) {
                        w.kindle(amount);
                    }
                }
            }
            ScriptCommand::ChillWarm { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWarmEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWarpMaxRange { name, value } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWarpChargeDuration { name, value } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Warp>(e) {
                        w.charge_duration = value;
                    }
                }
            }
            ScriptCommand::SetWarpCooldownDuration { name, value } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Warp>(e) {
                        w.cooldown_duration = value;
                    }
                }
            }
            ScriptCommand::SetWarpEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::ApplyWax { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Wax>(e) {
                        w.apply(amount);
                    }
                }
            }
            ScriptCommand::SetWaxMaxWax { name, value } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWaxEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::AdvanceWay { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Way>(e) {
                        w.advance(amount);
                    }
                }
            }
            ScriptCommand::RetreatWay { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWayEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::NourishWeal { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Weal>(e) {
                        w.nourish(amount);
                    }
                }
            }
            ScriptCommand::DepleteWeal { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWealEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::TireWeary { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Weary>(e) {
                        w.tire(amount);
                    }
                }
            }
            ScriptCommand::RestWeary { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWearyEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWeatherCloudCover { name, value } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Weather>(e) {
                        w.cloud_cover = value;
                    }
                }
            }
            ScriptCommand::SetWeatherPrecipitationIntensity { name, value } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Weather>(e) {
                        w.precipitation_intensity = value;
                    }
                }
            }
            ScriptCommand::SetWeatherTemperature { name, value } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWeatherFogDensity { name, value } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWeatherEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::BeginWeave { name } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Weave>(e) {
                        w.begin();
                    }
                }
            }
            ScriptCommand::HaltWeave { name } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWeaveEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SlipWeasel { name } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Weasel>(e) {
                        w.slip();
                    }
                }
            }
            ScriptCommand::SetWeaselEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::ApplyWeb { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::StruggleWeb { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWebEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::StrikeWelt { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Welt>(e) {
                        w.strike(amount);
                    }
                }
            }
            ScriptCommand::SalveWelt { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWeltEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::TraverseWend { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Wend>(e) {
                        w.traverse(amount);
                    }
                }
            }
            ScriptCommand::BacktrackWend { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWendEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::WaftWhiff { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Whiff>(e) {
                        w.waft(amount);
                    }
                }
            }
            ScriptCommand::DissipateWhiff { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWhiffEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::FancyWhim { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Whim>(e) {
                        w.fancy(amount);
                    }
                }
            }
            ScriptCommand::SettleWhim { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWhimEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::CrackWhip { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Whip>(e) {
                        w.crack(amount);
                    }
                }
            }
            ScriptCommand::SlackenWhip { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWhipEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SpinUpWhirl { name } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Whirl>(e) {
                        w.spin_up();
                    }
                }
            }
            ScriptCommand::SpinDownWhirl { name } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWhirlEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::BeatWhisk { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
                if let Some(e) = entity {
                    if let Some(mut w) = world.get_mut::<bsengine_core::Whisk>(e) {
                        w.beat(amount);
                    }
                }
            }
            ScriptCommand::SettleWhisk { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWhiskEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SoakWick { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SpendWick { name, amount } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
            ...
            ScriptCommand::SetWickEnabled { name, enabled } => {
                let entity = {
                    let mut q = world.query::<(Entity, &Name)>();
                    q.iter(world).find(|(_, n)| n.0 == name).map(|(e, _)| e)
                };
